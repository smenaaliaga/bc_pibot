...
def _extract_entities_from_state(
    ...
    # ``intent`` head del clasificador: campo plano (str), no dict. Se usa en
    # response.py para detectar consultas de NIVEL ("value") de forma semántica
    # en lugar de depender solo de regex sobre el texto crudo.
    intent_cls_raw = getattr(classification, "intent", None) if classification else None
    intent_cls = str(intent_cls_raw).strip().lower() if intent_cls_raw else None
