...
def _build_metric_priority_instruction(calc_mode: str) -> Optional[str]:
    ...
    if mode == "contribution":
        return (
            ...
            "1. En el PRIMER PARRAFO (apertura del agregado) comienza mencionando el PERIODO analizado "
            "(ej: 'En el 3er trimestre de 2025, ...') y reporta el agregado del indicador (PIB/IMACEC) "
            "como variación porcentual con la frase completa 'creció/disminuyó **X,X%** respecto al "
            "mismo período del año anterior'. La apertura SÍ usa '%' y SÍ usa verbos.\n"
            "2. En el LISTADO POR ACTIVIDAD (ranking) los datos son CONTRIBUCIONES expresadas en "
            "PUNTOS PORCENTUALES con signo explícito. La unidad obligatoria es 'pp' pegada al "
            "número, NO '%'. Cada actividad se reporta como pareja compacta '[Actividad] **±X,Xpp**'.\n"
            "3. Redondea SIEMPRE a 1 decimal: 0,54 → '+0,5pp'; -0,67 → '-0,7pp'; 0 → '0,0pp'.\n"
            "4. Ordena por VALOR ABSOLUTO de la contribución (mayor primero).\n"
            "5. Separa claramente contribuciones positivas de negativas en grupos distintos.\n"
            "6. REGLA DE SIGNO Y UNIDAD POR ACTIVIDAD (OBLIGATORIA): cada actividad listada "
            "debe escribirse con signo explícito y sufijo 'pp', sin verbo intermedio:\n"
            "   - si contribución < 0: escribe '**-X,Xpp**' (con signo menos pegado).\n"
            "   - si contribución > 0: escribe '**+X,Xpp**' (con signo más pegado).\n"
            "   - si contribución = 0: escribe '**0,0pp**'.\n"
            "7. SINÓNIMOS PERMITIDOS SOLO EN LA APERTURA DEL AGREGADO (no por actividad): "
            "para el agregado del indicador puedes usar 'creció/aumentó' (signo +) o "
            "'disminuyó/cayó' (signo -). El agregado se reporta en '%' (variación), NO en 'pp'. "
            "El listado por actividad NO usa estos verbos.\n"
            "8. PROHIBIDO usar '%' al lado del valor de una actividad listada en el ranking; "
            "debe ir 'pp' siempre.\n"
            "8.b. PROHIBIDO usar verbos ('disminuyó', 'creció', 'aumentó', 'cayó', 'subió', 'bajó') "
            "junto al valor de una actividad listada. La actividad se reporta como pareja "
            "compacta '[Actividad] [valor firmado]pp', sin verbo intermedio.\n"
            "8.c. PROHIBIDO añadir 'respecto al mismo período del año anterior' al lado del "
            "valor por actividad: el listado es compacto y NO repite la referencia temporal "
            "(esa frase pertenece SOLO a la apertura del agregado).\n"
            "8.d. CHEQUEO FINAL OBLIGATORIO ANTES DE RESPONDER: si en el listado por actividad "
            "aparece un porcentaje terminado en '%' (ej. '0,5%') o un verbo de variación junto "
            "a una actividad, debes reescribir esa línea con el formato '[Actividad] **±X,Xpp**'.\n"
            "8.e. RESPUESTA INVÁLIDA si en el listado por actividad aparece el patrón "
            "'X,X%' o frases como 'disminuyó X,X%' / 'creció X,X%' al describir una actividad.\n"
            "8.f. RESPUESTA INVÁLIDA si usa frases como 'contribuciones negativas de ...' o "
            "'incidencias negativas ... con -X,X%'. Reescribe con la plantilla compacta "
            "'[Actividad] **-X,Xpp**'.\n"
            "8.f.1. RESPUESTA INVÁLIDA si usa la frase 'a la baja' en el cuerpo del listado "
            "por actividad. La señalización viene del SIGNO numérico, no de adjetivos.\n"
            "8.g. PLANTILLA OBLIGATORIA POR ACTIVIDAD EN CONTRIBUCIÓN: "
            "'[Actividad] **±X,Xpp**'. Ejemplos válidos: "
            "'Servicios personales **-1,1pp**'; 'Construcción **+0,5pp**'; "
            "'Comercio **0,0pp**'.\n"
            "8.g.1. REGLA OBLIGATORIA POR CADA VALOR DE ACTIVIDAD: toda cifra de contribución "
            "(positiva, negativa o cero) debe ir con signo explícito y sufijo 'pp' pegado al "
            "número, dentro de los asteriscos de negrita. Si una actividad aparece como "
            "'**0,3pp**' sin signo, debe corregirse a '**+0,3pp**'.\n"
...
def _is_level_only_query(question: str, entities_ctx: Dict[str, Any]) -> bool:
    ...
    Jerarquía de decisión (de mayor a menor prioridad):
      ...
      1. Override léxico de variación: si el texto contiene "variación",
         "crecimiento", "cayó", etc. → False (siempre).
      2. Override semántico de variación: si el clasificador resolvió
         ``intent='variation'`` o ``calc_mode`` ∈ {contribution, share,
         prev_period} → False (la pregunta no es de nivel aunque el texto
         contenga "valor"/"cifra").
      3. Señales estructurales fuertes desde entidades: ``price_ent='co'``,
    ...
    # 1. Override léxico de variación.
    if _VARIATION_REQUEST_RE.search(text_norm):
        return False

    # 2. Override semántico desde clasificador.
    intent_cls = str(entities_ctx.get("intent_cls") or "").strip().lower()
    calc_mode = str(
        entities_ctx.get("calc_mode_cls") or entities_ctx.get("calc_mode") or ""
    ).strip().lower()
    if intent_cls == "variation":
        return False
    if calc_mode in {"contribution", "share", "prev_period"}:
        return False

    # 3. Señales estructurales fuertes (entidades + texto explícito).
    price_ent = str(entities_ctx.get("price_ent") or entities_ctx.get("price") or "").strip().lower()
    ...
    if indicator_ent == "pib_per_capita" or activity_ent == "per_capita":
        return True
    if re.search(r"\bnominal(?:es)?\b", text_norm):
        return True
