...
def make_data_node(memory_adapter: Any):
    def data_node(state: AgentState, *, writer: Optional[StreamWriter] = None):
        ...
        _merge_entity_fields(
            primary_entity,
            {
                "indicator": getattr(getattr(result, "indicator", None), "label", None),
                "activity": getattr(getattr(result, "activity", None), "label", None),
                "seasonality": getattr(getattr(result, "seasonality", None), "label", None),
            },
            overwrite=True,
        )
        _merge_entity_fields(
            primary_entity,
            {
                "metric_type_cls": getattr(getattr(result, "metric_type", None), "label", None),
                "calc_mode_cls": getattr(getattr(result, "calc_mode", None), "label", None),
                "req_form_cls": getattr(getattr(result, "req_form", None), "label", None),
                "frequency_cls": getattr(getattr(result, "frequency", None), "label", None),
            },
            overwrite=True,
        )
