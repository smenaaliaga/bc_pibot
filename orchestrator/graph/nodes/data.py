...
def make_data_node(memory_adapter: Any):
    def data_node(state: AgentState, *, writer: Optional[StreamWriter] = None):
        ...
        calc_mode = getattr(classification, "calc_mode", None) or {}
        activity = getattr(classification, "activity", None) or {}
        region = getattr(classification, "region", None) or {}
        investment = getattr(classification, "investment", None) or {}
        req_form = getattr(classification, "req_form", None) or {}
        ...
        logger.info("[DATA_NODE !!!] calc_mode=%s", calc_mode)
        logger.info("[DATA_NODE !!!] activity=%s", activity)
        logger.info("[DATA_NODE !!!] region=%s", region)
        logger.info("[DATA_NODE !!!] investment=%s", investment)
        logger.info("[DATA_NODE !!!] req_form=%s", req_form)
        logger.info("[DATA_NODE !!!] fallback_entities=%s", fallback_entities)
        logger.info("[DATA_NODE !!!] fallback_normalized=%s", fallback_normalized)
        logger.info("[DATA_NODE !!!] =========================================================")
