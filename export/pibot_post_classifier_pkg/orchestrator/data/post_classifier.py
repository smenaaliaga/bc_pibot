...
#
# EXPLICACIÓN DEL PROCESO
#   En consultas de contribución individual de IMACEC, si no se especificó
#   actividad, se asigna activity_ent='imacec' como default para resolver
#   la serie agregada. En consultas de nivel/valor general NO se fuerza esta
#   actividad para evitar sesgos hacia cuadros no solicitados.
#
# Input   : ent.indicator_ent='imacec', ent.calc_mode_cls='contribution',
#           ent.activity_ent is None
...
#   3) Si la consulta pide nivel, por defecto responder en 'co'.
#   4) Si la consulta habla de datos reales, forzar serie original en 'co'.
#   5) Fallback general: 'enc'.
#
# Input   : ent.price_ent, ent.question
# Output  : ent.price, ent.calc_mode_cls (caso datos reales)
# ============================================================================
...
class Rule09_NivelesNominales:
    ...
    def assign_price(cls, ent: ResolvedEntities) -> None:
        ...
        # Corrección de desambiguación: cuando el clasificador normaliza
        # "pib real" como IMACEC, forzar indicador PIB.
        if cls.PIB_REAL_RE.search(q) and str(ent.indicator_ent or "").strip().lower() == "imacec":
            ent.indicator_ent = "pib"
            # Si venía frecuencia mensual por arrastre de IMACEC, al menos
            # subir a trimestral/anual según señal léxica de años.
            if str(ent.frequency_ent or "").strip().lower() == "m":
                ent.frequency_ent = "a" if cls.YEAR_WORD_RE.search(q) else "q"
            _trace(ent, cls.CAT, "pib_real_disambiguation", f"indicator={ent.indicator_ent}, frequency={ent.frequency_ent}")

        # 1) Si el clasificador trajo precio explícito, respetar.
        if ent.price_ent in ("co", "enc"):
            ent.price = ent.price_ent
            _trace(ent, cls.CAT, "assign_price", f"price={ent.price} (from price_ent)")
            return

        # 2) Nominal / precios corrientes, y PIB real bajo la misma política
        #    de precios corrientes solicitada.
        ...
        # 3) Consulta de nivel: default a precios corrientes.
